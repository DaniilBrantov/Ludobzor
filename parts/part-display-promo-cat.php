...
<div class="container">
    ...
    <div class="onlinecasino_item onlineslot_item" data-k="8" data-i="2012" id="bx_<?php echo get_the_ID(); ?>">
        <div class="row vertical-gutter">
            <?php 
            // Инициализируем запрос для получения всех постов типа 'free_games'
            $free_games_query = new WP_Query(array(
                'post_type' => 'free_games',
                'posts_per_page' => 3, // Получаем все посты
            ));
            ...
            if ($free_games_query->have_posts()) : ?>
                <?php while ($free_games_query->have_posts()) : $free_games_query->the_post(); 
                    ...
                    $game_image = get_field('фото_для_главной_webp'); // Картинка игры
                    $casino_name = get_field('казино'); // Название казино из кастомного поля
                    // Найдем пост типа 'online_casino' с тем же названием казино
                    $casino_query = new WP_Query(array(
                        'post_type' => 'online_casino',
                        'meta_query' => array(
                            array(
                                'key' => 'чистое_название', // Предполагается, что в 'online_casino' есть поле 'название_казино'
                                'value' => $casino_name,
                                'compare' => '='
                            )
                        ),
                        'posts_per_page' => 1, // Предполагаем, что будет одно казино с таким названием
                    ));
                            ...
                            <a href="javascript:;" class="copy_promocode_link"
                                ...
                                <svg width="12" height="12" viewBox="0 0 12 12" fill="none"
                                    ...
                                    <path fill-rule="evenodd" clip-rule="evenodd"
                                        ...
                                        fill="white"></path>
                                </svg> <?php echo esc_html($promocode); ?>
                            </a>
                        </span>
                        </div>
                    </div>
                    ...
                    <?php wp_reset_postdata(); // Сбрасываем данные казино для следующей итерации ?>
                    <?php endif; // Закрываем проверку для $casino_query ?>
